...
/**
 * OpenBoundaryCheck
 * This check flags boundaries that should be closed polygons but are not.
 *
 * @author v-garei
 */
public class OpenBoundaryCheck extends BaseCheck<Long>
{
    ...
    private static final List<String> FALLBACK_INSTRUCTIONS = Collections.singletonList(OPEN_BOUNDARY_INSTRUCTIONS);
    ...
    {
        Map<String, String> osmKeys = object.getOsmTags();
    ...
    protected Optional<CheckFlag> flag(final AtlasObject object)
    {
        ...
        if (hasAllMembersLoaded(relation))
        {
            try {
            ...
            catch (final MultiplePolyLineToPolygonsConverter.OpenPolygonException exception)
            {
                ...
                final Set<String> latllonSet = new HashSet<>();

                for (Location openLocation : openLocations)
                {
                ...
                if (!openLocations.isEmpty())
                {
                    return Optional.of(this.createFlag(object,
                            this.getLocalizedInstruction(0,
                                    relation.getOsmIdentifier(), memberIds, latllonSet)));
    ...
    /**
     * Checks to make sure relation is fully loaded by checking if the
     * all known members count is the same as the amount of members the relation in question has
     * @param relation relational boundary in question
