...
public class LevelCrossingOnRailwayCheck extends BaseCheck<Long>
{
    ...
    private static final String NODE_CROSSING_MISSING_LC_TAG = "The node (OSM ID: {0,number,#}) has a "
            + "`railway=crossing` tag but should be `railway=level_crossing` tag. "
    ...
    private static final String NODE_INVALID_LC_TAG_CYCLE_ONLY_HIGHWAY = "The node (OSM ID: {0,number,#}) has "
            + "`railway=level_crossing` tag, but there is no car navigable highway at this intersection, only "
            + "cycleway highway(s). "
    ...
    private static final List<String> FALLBACK_INSTRUCTIONS = Arrays.asList(INVALID_TAGGED_OBJECT,
            NODE_CROSSING_MISSING_LC_TAG, NODE_MISSING_LC_TAG, NODE_INVALID_LC_TAG_NO_HIGHWAY,
