...
public class InconsistentRoadClassificationCheck extends BaseCheck<Long>
{
    ...
    public InconsistentRoadClassificationCheck(final Configuration configuration)
    {
        ...
        this.maximumEdgeLength = Distance.meters(
                configurationValue(configuration, "maximum.edge.length",
                        MAXIMUM_EDGE_LENGTH_DEFAULT));
    ...
    public boolean validCheckForObject(final AtlasObject object)
    {
        if (object instanceof Edge && !isFlagged(object.getOsmIdentifier()))
        {
            ...
            return edge.highwayTag().isMoreImportantThanOrEqualTo(this.minimumHighwayType)
                    ...
                    edge.outEdges().stream().noneMatch(connectedEdge ->
                            edge.getMasterEdgeIdentifier()
                                    != connectedEdge.getMasterEdgeIdentifier()
    ...
    protected Optional<CheckFlag> flag(final AtlasObject item)
    {
        ...
        final List<Tuple<Edge, Set<Edge>>> inconsistentEdgeTuples = this.findInconsistentEdges(
                edge);
        if (!inconsistentEdgeTuples.isEmpty())
        {
            ...
            inconsistentEdgeTuples.forEach(inconsistentEdgeTuple ->
            {
                ...
                final Set<Edge> inEdges = inconsistentEdge.inEdges().stream().filter(
                        Edge::isMasterEdge).collect(Collectors.toSet());
                final Set<Edge> outEdges = inconsistentEdge.outEdges().stream().filter(
                        Edge::isMasterEdge).collect(Collectors.toSet());
                        ...
                        || edgeLength.isLessThanOrEqualTo(this.maximumEdgeLength))
                {
                    flag.addObject(inconsistentEdge,
                            this.getLocalizedInstruction(1, inconsistentEdge.getOsmIdentifier(),
                                    inconsistentEdge.highwayTag()));
                    ...
                    {
                        flag.addObject(followingEdge,
                                this.getLocalizedInstruction(2, followingEdge.getOsmIdentifier(),
                                        followingEdge.highwayTag()));
    ...
    private boolean areInTheSimilarDirection(final Edge edge, final Edge anotherEdge)
    {
        ...
        return finalHeading.isPresent() && initialHeading.isPresent()
                && finalHeading.get().difference(initialHeading.get()).isLessThan(
                this.similarDirectionDifferenceThreshold);
    }
    ...
    private Stream<Edge> connectionsSimilarToReferenceEdge(final HighwayTag referenceHighwayType,
            final Edge edge)
    {
        return edge.outEdges().stream().filter(connectedEdge ->
                referenceHighwayType.isOfEqualClassification(connectedEdge.highwayTag())
    ...
    private List<Tuple<Edge, Set<Edge>>> findInconsistentEdges(final Edge referenceEdge)
    {
        ...
        final Map<Boolean, List<Edge>> edgesAreProblematicLinks = referenceEdge.outEdges().stream().filter(
                this.allConnectedEdgesFilter(referenceEdge, referenceHighwayType)).collect(
        ...
        return Stream.concat(
                ...
                edgesAreProblematicLinks.get(true).stream().map(
                        this.getSimilarEdgesTuple(referenceHighwayType,
                                middleEdge -> endEdge -> true)),
                // Only flag other edges if they pass some other criteria
                edgesAreProblematicLinks.get(false).stream().filter(
                        this.nonLinkEdgesFilter(referenceHighwayType)).map(
                        this.getSimilarEdgesTuple(referenceHighwayType,
     ...
     * of out edges from E which are consistent with referenceHighwayType, filtered by
     * tuplesFilter.
     *
     * @param referenceHighwayType
            ...
            final Function<Edge, Predicate<Edge>> tuplesFilter)
    {
        ...
        {
            final Set<Edge> inEdges = connectedEdge.inEdges().stream().filter(
                    Edge::isMasterEdge).collect(Collectors.toSet());
            final Set<Edge> outEdges = connectedEdge.outEdges().stream().filter(
                    Edge::isMasterEdge).collect(Collectors.toSet());
            // Get similar connections using first edge identifier and type as reference
            final Set<Edge> similarConnections = this.connectionsSimilarToReferenceEdge(
                    referenceHighwayType, connectedEdge).filter(
                    tuplesFilter.apply(connectedEdge)).collect(Collectors.toSet());
            if (!similarConnections.isEmpty() && ((inEdges.size() <= TWO_CONNECTED_EDGES
                    && outEdges.size() <= TWO_CONNECTED_EDGES)
                    || connectedEdge.asPolyLine().length().isLessThanOrEqualTo(
                    this.maximumEdgeLength)))
            {
    ...
    {
        return inconsistency.start().outEdges().stream().anyMatch(
                edge -> !edge.equals(inconsistency) && edge.end().equals(inconsistency.end())
    ...
    {
        return edge.outEdges().stream().anyMatch(connectedEdge ->
                edge.highwayTag().isOfEqualClassification(connectedEdge.highwayTag())
                        && this.areInTheSimilarDirection(edge, connectedEdge));
    }
    ...
    {
        return edge.highwayTag().isMoreImportantThan(referenceHighwayType) && (
                edge.length().isGreaterThanOrEqualTo(this.longEdgeThreshold)
                        || edge.connectedNodes().stream().allMatch(
                        node -> node.connectedEdges().stream().anyMatch(connectedEdge ->
                                edge.getMasterEdgeIdentifier()
                                        != connectedEdge.getMasterEdgeIdentifier()
                                        && edge.highwayTag().isOfEqualClassification(
                                        connectedEdge.highwayTag()))));
    }
    ...
    {
        return start.connectedNodes().stream().anyMatch(
                node -> inconsistent.connectedNodes().contains(node)
    ...
    {
        return edge -> !this.isPartOfALongerRoad(edge, referenceHighwayType) && !this.isBypassed(
                edge, referenceHighwayType) && !this.isLongLessImportantEdge(edge,
                referenceHighwayType);
    }
