...
public class AtGradeSignPostCheck extends BaseCheck<String>
{
    ...
    private static final String DEFECTIVE_DESTINATION_SIGN_RELATION_INSTRUCTION = "Node {0,number,#} form an at-grade junction. It is part of destination sign relation(s):"
            + "{1}"
            + "but the relation(s) are missing \"destination\" tags.";
    ...
    private static final List<String> FALLBACK_INSTRUCTIONS = Arrays.asList(NO_DESTINATION_SIGN_RELATION_INSTRUCTION,
            DEFECTIVE_DESTINATION_SIGN_RELATION_INSTRUCTION, INCOMPLETE_DESTINATION_RELATION_INSTRUCTION, MISSING_DESTINATION_SIGN_INSTRUCTION);
    ...
    protected Optional<CheckFlag> flag(final AtlasObject object)
    {
        ...
        final Set<Edge> inEdges = intersectingNode.inEdges().stream()
                .filter(inEdge -> inEdge.isMasterEdge()
                        && HighwayTag.highwayTag(inEdge).isPresent()
        ...
        final Set<Edge> outEdges = intersectingNode.outEdges().stream()
                .filter(outEdge -> outEdge.isMasterEdge()
                        ...
                        && this.highwayFilter.test(outEdge)).collect(Collectors.toSet());
        ...
        {
            final String highwayTypeOfCurrentEdge = HighwayTag.highwayTag(inEdge).get().getTagValue();
            if(this.connectedHighwayTypes.containsKey(highwayTypeOfCurrentEdge))
            {
                ...
                final Set<AtlasEntity> filteredOutEdges = outEdges.stream()
                        .filter(outEdge -> LevelTag.areOnSameLevel(inEdge, outEdge)
                                    && LayerTag.areOnSameLayer(inEdge, outEdge)).collect(Collectors.toSet());
                // If there are at least two out edges, filter these edges based on their highway types.
                // Check if the highway type of out edge is one of the valid connected highway types based on
                // the incoming edge
                if(filteredOutEdges.size() >=2)
                {
                    final Set<AtlasEntity> filteredOutEdgesBasedOnHighwayTypes = filteredOutEdges.stream()
                            .filter(atlasEntity -> {
                    ...
                    if(filteredOutEdgesBasedOnHighwayTypes.stream().anyMatch(JunctionTag::isRoundabout))
                    {
                        roundAboutInEdgeToOutEdgeMap.put(inEdge, filteredOutEdgesBasedOnHighwayTypes);
                    }
        ...
        Set<AtlasEntity> entitiesToBeFlagged = new HashSet<>();
        // Collect all destination sign relations, the node is member of
        final Optional<Set<Relation>> destinationSignRelations = this.getParentDestinationRelations(intersectingNode);

        // If the node is part of destination sign relation, check if destination tag of the relation is missing or
        // if there are any missing relations that the node could be part of based on from and to edges
        if(destinationSignRelations.isPresent())
        {
            final Set<Relation> defectiveDestinationSignRelations = this.getRelationsWithMissingDestinationTag(destinationSignRelations.get());
            ...
            else if(!roundAboutInEdgeToOutEdgeMap.isEmpty())
            {
                roundAboutInEdgeToOutEdgeMap.forEach((inEdge, setOfOutEdge) ->
                {
                    ...
                    {
                        final Set<AtlasEntity> roundaboutEdges = new SimpleEdgeWalker((Edge)roundaboutEdge.get(), this.isRoundaboutEdge())
                                .collectEdges().stream().map(AtlasEntity.class::cast).collect(
                                        Collectors.toSet());
            ...
            {
                // If there are any missing destination sign relation that the node should be part of, flag it
                final Set<AtlasEntity> connectedEdgesNotFormDestinationRelation = this.getConnectedEdgesNotFormDestinationRelation(inEdgeToOutEdgeMap, destinationSignRelations.get());
        ...
        else if(roundAboutInEdgeToOutEdgeMap.isEmpty())
        {
            ...
            roundAboutInEdgeToOutEdgeMap.forEach((inEdge, setOfOutEdge) ->
            {
                ...
                {
                    final AtlasEntity exitEdge = roundaboutExitEdge.get();
                    if(!OneWayTag.isExplicitlyTwoWay(exitEdge) && !(roundaboutEdge.get().tag(DestinationTag.KEY) != null || intersectingNode.tag(DestinationTag.KEY) != null
        ...
        return instructionIndex == -1 ? Optional.empty() : Optional.of(this.createFlag(entitiesToBeFlagged,
                this.getLocalizedInstruction(instructionIndex, intersectingNode.getIdentifier(),
                        new StringList(identifiers).join(", "))));
    }

    private Set<Relation> getRelationsWithMissingDestinationTag(final Set<Relation> destinationSignRelations)
    {
        return destinationSignRelations.stream().filter(relation -> relation.tag(DestinationTag.KEY) == null).
                collect(Collectors.toSet());
    }

    /**
     *
     * @param inEdgeToOutEdgeMap inEdge to outEdge map
     * @param destinationSignRelations destinationSignRelations
     * @return
     */
    ...
    {
        return inEdgeToOutEdgeMap.entrySet().stream()
                .flatMap(atlasEntitySetEntry -> {
                    final AtlasEntity key = atlasEntitySetEntry.getKey();
                    final Optional<Set<AtlasEntity>> atlasEntities = this
                            .connectedEdgesNotPartOfRelation(key, inEdgeToOutEdgeMap.get(key),
                                    destinationSignRelations);
                    return atlasEntities.map(Stream::of).orElseGet(Stream::empty);
                }).flatMap(Collection::stream).collect(Collectors.toSet());
    }

    private Optional<Set<AtlasEntity>> connectedEdgesNotPartOfRelation(final AtlasEntity fromEdge, final Set<AtlasEntity> toEdges,
            final Set<Relation> destinationSignRelations)
    {
     ...
     *
     * @param atlasEntity any {@link AtlasEntity}
    ...
    {
        return node.valence() >= 3 && node.inEdges().stream().anyMatch(this.highwayFilter)
